...
QFile Size("C:/QtRepos/TravellerSalesMan/MatrixSize.txt");
QTextStream streamSize(&Size);
QFile Matrix("C:/QtRepos/TravellerSalesMan/Matrix.txt");
QTextStream streamMatrix(&Matrix);

int len;                                        // Кол-во узлов в графе
int Radius = 300;                               // Радиус графа
int radius = 40;                                // Радиус узла
double pi = 3.1415;
int HeightText = 30;
int xa = 600;                                   // Отступ от вертикали
int ya = 400;                                   // Отступ от горизонтали
double angle = pi / 8;                          // Угол раствора стрелки
int larrow = 30;                                // Длина кончика стрелки
...
MyPainter::MyPainter()
{
    front = QBrush(QColor(255, 170, 0));
    back = QBrush(QColor(0, 200, 200));
